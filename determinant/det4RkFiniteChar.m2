...
-- The result is that the determinant is 2^32 in the case 2,3,4.
-- In the case 1, the determinant is - 2^20 * (x-1) * (x-2)^4 * (x-4)^4.
-- Hence we consider the case 1 in the below.

-- Let M = koszCase_1.
M = koszCase_1;

-- We construct 91 by 91 submatrices M' and M'' of M.
M' = submatrix'(M, {6, 14, 18, 38, 46}, {35, 47, 55, 83, 95});
M'' = submatrix'(M, {2, 6, 22, 30, 34}, {31, 35, 59, 71, 79});

-- When x=1,
stdio << "Case 1, when x=1, det M'=" << factor sub(det sub(M',x=>1),ZZ) << endl;     -- the submatrix M' has rank 91

-- When x=2,
stdio << "Case 1, when x=2, det M''=" << factor sub(det sub(M'',x=>2),ZZ) << endl;   -- the submatrix M'' has rank 91

-- When x=4,
stdio << "Case 1, when x=4, det M'=" << factor sub(det sub(M',x=>4),ZZ) << endl;     -- the submatrix M' has rank 91
